export const userManagementForm = [
    {
        button: {
            title: 'Add or Modify Role',
        },
        info: {
            description: 'You can add or modify role to set the authority for each user.',
            title: 'Role',
        },
        label: 'Active Role',
        section: 'role',
    },
    {
        button: {
            icon: 'lock',
            title: 'Add or Modify Role',
        },
        info: {
            description:
                'Manage the permission for each role. You can give Access Menu and User Control.',
            title: 'Permission',
        },
        section: 'role',
    },
    {
        button: {
            title: 'Add or Modify User',
        },
        info: {
            description:
                'List of User in this system. You can add or modify each user except administrator.',
            title: 'User',
        },
        label: 'Active User',
        section: 'user',
    },
];
